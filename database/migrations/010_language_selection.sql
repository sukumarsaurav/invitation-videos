...
INSERT INTO languages (code, name, native_name, script, display_order) VALUES
...
INSERT INTO template_thumbnails (template_id, language_code, thumbnail_url, is_primary, display_order)
...
-- Common translations for standard field labels
-- This will be inserted when admin adds translations via admin panel

-- Add translation_price to settings or use hardcoded value
-- For now, we'll use ₹99 / $1.99 as hardcoded in the application

-- Remove language from mega menu categories (handled in PHP code)
-- Language filter will now be on template detail page instead

-- Add language preference columns to orders table
ALTER TABLE orders 
    ADD COLUMN selected_language VARCHAR(10) DEFAULT 'en' AFTER customization_data,
    ADD COLUMN translation_mode ENUM('self', 'translate') DEFAULT 'self' AFTER selected_language,
    ADD COLUMN translation_fee DECIMAL(10,2) DEFAULT 0.00 AFTER translation_mode;
