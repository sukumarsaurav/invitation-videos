...
// Note: Templates now use GenericTemplate composition with JSON-based template definitions
// The visual editor (template-editor.php) manages the template_definition JSON
                        ...
                        <tr class="hover:bg-slate-50:bg-white/5 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="/admin/template-editor.php?id=<?= $tpl['id'] ?>"
                                        class="p-2 rounded-lg hover:bg-purple-50 text-slate-500 hover:text-purple-600 transition-colors"
                                        title="Visual Editor">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <?php if ($action === 'edit' && $templateId): ?>
            <a href="/admin/template-editor.php?id=<?= $templateId ?>"
                class="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white font-bold py-2.5 px-5 rounded-lg shadow-sm transition-all">
                <span class="material-symbols-outlined text-lg">movie_edit</span>
                Visual Editor
            </a>
        <?php endif; ?>
    ...
    <form method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
                ...
                <p class="text-sm text-slate-500 mb-4">Configure video rendering with the Visual Editor. Template
                    definitions are stored as JSON.</p>

                <!-- Hidden field to store GenericTemplate as composition ID -->
                <input type="hidden" name="remotion_composition_id" value="GenericTemplate">

                <?php
                $hasTemplateDefinition = !empty($template['template_definition']);
                $templateDef = $hasTemplateDefinition ? json_decode($template['template_definition'], true) : null;
                $slideCount = $templateDef ? count($templateDef['slides'] ?? []) : 0;
                ?>

                <?php if ($hasTemplateDefinition && $slideCount > 0): ?>
                    <!-- Template has JSON definition -->
                    <div class="p-4 rounded-lg bg-green-50 border border-green-200 mb-4">
                        <div class="flex items-center gap-3">
                            <span class="material-symbols-outlined text-green-600 text-2xl">check_circle</span>
                            <div class="flex-1">
                                <p class="text-sm text-green-700 font-medium">Template Configured</p>
                                <p class="text-xs text-green-600 mt-0.5">
                                    <?= $slideCount ?> slide<?= $slideCount !== 1 ? 's' : '' ?> •
                                    <?= $templateDef['fps'] ?? 30 ?>fps •
                                    <?= $templateDef['width'] ?? 1080 ?>×<?= $templateDef['height'] ?? 1920 ?>
                                </p>
                            </div>
                            <a href="/admin/template-editor.php?id=<?= $templateId ?>"
                                class="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium py-2 px-4 rounded-lg transition-colors">
                                <span class="material-symbols-outlined text-lg">movie_edit</span>
                ...
                <?php else: ?>
                    <!-- No template definition yet -->
                    <div class="p-4 rounded-lg bg-amber-50 border border-amber-200 mb-4">
                        <div class="flex items-center gap-3">
                            <span class="material-symbols-outlined text-amber-600 text-2xl">movie_edit</span>
                            <div class="flex-1">
                                <p class="text-sm text-amber-700 font-medium">Template Not Configured</p>
                                <p class="text-xs text-amber-600 mt-0.5">
                                    Use the Visual Editor to create slides, add text/image layers, and set animations.
                                </p>
                            </div>
                            <?php if ($action === 'edit' && $templateId): ?>
                                <a href="/admin/template-editor.php?id=<?= $templateId ?>"
                            ...
                            <?php else: ?>
                                <span class="text-xs text-amber-600 italic">Save template first to access Visual Editor</span>
                            <?php endif; ?>
                        </div>
                    </div>
                <?php endif; ?>
